...
$ETHERCAT -l $ETHER_INTERFACE 00:00:00:00:00:00 > ${BASH_SOURCE%/*}/received.txt &

# Give reciever time to start up and bind socket.
sleep 0.5s

# Send ethercat's source code over to it as test material.
cat ${BASH_SOURCE%/*}/../src/main.rs | $ETHERCAT -O 500 $ETHER_INTERFACE 00:00:00:00:00:00 > /dev/null
